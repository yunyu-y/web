// Copyright 2019 GreenSock Inc. See http://greensock.com/gsap/
;(function ($) {
	var htmlLink = "/wp-content/themes/greensock/includes/EaseVisualizer/EaseVisualizer.html";
	var id = 0;
	var timeline;
	var $menuEases;
	var highlightTween;
	...
	var customStrings = {
		...
		"switchine.out":     "M0,0,C0.266,0.412,0.436,0.654,0.565,0.775,0.609,0.816,0.78,1,1,1",
		...
		init:    function (options) {
			...
			return this.each(function (index) {
				...
				if (!data) {
					...
					if (window.easeVisualizerHTML) {
						loaded(window.easeVisualizerHTML, vis);
					} else {
	...
	function parseEaseClass(name, short) {
		...
		if (short) {
			return (name === "Rough") ? "RoughEase" : (name === "Stepped") ? "SteppedEase" : name;
		}
	...
	function highlightMenuEase(name) {
		...
		if (highlightTween) { //a simple, performant way to unhighlight the previous ease.
			if (highlightTween.targets()[0].textContent !== name) { //if it's the same target, it means the user clicked on the already-highlighted ease, so do nothing.
				gsap.to(highlightTween.targets()[0], {duration: 0.2, backgroundColor: "rgba(0,0,0,0)", color: "#777", clearProps: "backgroundColor,color"});
			} else {
				return true;
			}
		}
		...
		while (--i > -1) {
			if ($menuEases[i].textContent === name) {
				highlightTween = gsap.to($menuEases[i], {duration: 0.2, backgroundColor: "#88CE02", color: "black"});
			}
		}
	...
	function customMode(vis, enabled) {
		if (vis.custom !== enabled) {
			...
			if (enabled && vis.find(".target_select").val() === "graph") {
					...
					vis.editor = new PathEditor(path, {
						...
						onUpdate:   function () {
							var hadError = hasError,
								rawPath = this._rawPath || this._bezier, //note: changed _bezier to _rawPath around GSAP 2.1.0
				...
				gsap.to(".ease-instructions", {duration: 0.2, autoAlpha: 0});
			}
		}
	}
	...
	function checkVertical(vis) { //elastic.out needs more room up top.
		...
		var tall = (target_select === "graph" && (main_ease_class_select === "CustomEase" || (main_ease_class_select === "Elastic" || main_ease_class_select === "Back" || (main_ease_class_select === "RoughEase" && vis.find(".rough_ease_class_select").val() === "Elastic" && vis.find(".rough_ease_type_select").val() === "out"))));
		if (tall) {
	...
	function refreshTween(vis) {
		...
		switch (c) {
			...
			case "RoughEase":
				...
				var template = rc + "." + vis.find(".rough_ease_type_select").val();
	...
	function runGraphVis(vis) {
		...
		var offset = 0.2;
		var graphTL = timeline = gsap.timeline({delay: 0.1, defaults: {duration: d}}); //delay slightly to give the CPU time to breathe after all the setup (avoid jank)
		graphTL.add("start", offset);

		var number = vis.find(".progress_number");
		graphTL.to({p: 0}, {
			ease: "none", p: 1, onUpdate: function () {
				number.text(this.targets()[0].p.toFixed(2));
			}
		}, "start");

		graphTL.fromTo(vis.find(".graph_line"), {autoAlpha: 0}, {duration: 0.0001, autoAlpha: 1}, "start");

		graphTL.fromTo(vis.find("#graph_path_reveal rect"), {attr: {width: 0}}, {attr: {width: 500}, ease: "none"}, "start");
		graphTL.fromTo(vis.find(".graph_liney"), {attr: {x1: 0, x2: 0}}, {attr: {x1: 500, x2: 500}, ease: "none"}, "start");
		graphTL.fromTo(vis.find(".progress_joint"), {top: "100%"}, {top: "0%", ease: ease}, "start");
		graphTL.fromTo(vis.find(".graph_linex"), {attr: {y1: 500, y2: 500}}, {attr: {y1: 0, y2: 0}, ease: ease}, "start");

		graphTL.fromTo(vis.find(".horizontal .progress_fill"), {scaleX: 0, transformOrigin: "left center"}, {ease: "none", scaleX: 1}, "start");
		graphTL.fromTo(vis.find(".vertical .progress_fill"), {scaleY: 0, transformOrigin: "left bottom"}, {ease: ease, scaleY: 1}, "start");

		graphTL.to(vis.find(".graph_line"), {duration: 0.07, autoAlpha: 0});
	...
	function runClockVis(vis) {
		...
		var clockTL = timeline = gsap.timeline();
	...
	function runBoxVis(vis) {
		...
		var boxTL = timeline = gsap.timeline();
		...
		boxTL.fromTo(vis.find(".horizontal .progress_fill"), d, {scaleX: 0, transformOrigin: "left center"}, {ease: "none", scaleX: 1, force3D: true}, "start");
	...
	function createGraph(vis) {
		var name = vis.find(".main_ease_class_select").val(),
			...
			precision = (name === "Stepped" || name === "Bounce" || name === "Elastic") ? 3 : 1,
		...
		if (customString) {
			...
			simplified = ease.rawBezier || ease.custom ? path : PathEditor.simplifySVG(path, {tolerance: (precision === 1) ? 3 : 1, cornerThreshold: (mainEase === "Bounce") ? 130 : (mainEase === "Stepped" || mainEase === "Rough") ? 180 : 0});
...
jQuery(".ease-visualizer").each(function () {
